...
A foundational Kotlin convention plugin. Applies `org.jetbrains.kotlin.jvm` and configures the JVM toolchain, compiler arguments, and standard repositories. It is the shared baseline used by `io.github.denis-markushin.kotlin-library` and `io.github.denis-markushin.spring-service`; you can also apply it directly to any Kotlin/JVM project that does not need the higher-level conventions.
...
> If you also apply `io.github.denis-markushin.kotlin-library` or `io.github.denis-markushin.spring-service`, do not apply this plugin separately — it is already pulled in.
